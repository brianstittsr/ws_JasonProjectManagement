...
export class AuthService {
  ...
  static async registerWithEmail(
    ...
    try {
      ...
      const userData: Omit<User, 'uid'> = {
        email: firebaseUser.email || email, // Use provided email if Firebase email is null
  ...
  static async signInWithGoogle(): Promise<User> {
    try {
      ...
      if (isNewUser) {
        const userData: Omit<User, 'uid'> = {
          email: firebaseUser.email || `user-${firebaseUser.uid}@example.com`, // Fallback email if null
