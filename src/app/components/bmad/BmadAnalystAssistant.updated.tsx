...
import { ArchonService } from '../../services/archon.updated';
...
const BmadAnalystAssistant: React.FC<BmadAnalystAssistantProps> = ({ 
  onClose, 
  archonService,
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      ...
      content: useFallbackMode 
        ? 'Welcome to the BMAD Analyst Assistant. I\'m operating in fallback mode without access to the Archon knowledge base. I can still help with market research, brainstorming, competitive analysis, and creating project briefs based on my general knowledge. How can I assist you today?'
        : 'Welcome to the BMAD Analyst Assistant. I can help you with market research, brainstorming, competitive analysis, and creating project briefs. I have access to the Archon knowledge base with content tagged "resbyte". How can I assist you today?'
    }
  ]);
  const [input, setInput] = useState('');
  const [isProcessing, setIsProcessing] = useState(false);
  const [useRag, setUseRag] = useState(!useFallbackMode);
  ...
  // Update RAG state when fallback mode changes
  useEffect(() => {
    setUseRag(!useFallbackMode && !!archonService);
  }, [useFallbackMode, archonService]);
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      if (archonService && useRag && !useFallbackMode) {
        // Use RAG with Archon
        try {
          ...
          setError('Failed to search knowledge base. Falling back to general responses.');
          
          // Fall back to non-RAG response
          assistantMessage = {
            id: `assistant-${Date.now()}`,
            role: 'assistant',
            content: "I encountered an issue accessing the knowledge base. Let me provide a general response based on my training.\n\n" + 
                    generateAnalystResponse(input)
          };
        }
      } else {
  ...
  const generateResponseWithRAG = async (query: string, searchResults: any[]) => {
    try {
      ...
      if (archonService && !useFallbackMode) {
        try {
          const emailContent = {
          ...
          console.error('Error generating response with Archon:', error);
    ...
    <ErrorBoundary fallback={
      <Card className="flex flex-col h-[calc(100vh-12rem)]">
        <CardHeader>
          ...
          <CardDescription>An error occurred while loading the assistant</CardDescription>
        </CardHeader>
        <CardContent className="flex-1 flex items-center justify-center">
          <div className="text-center">
            ...
            <p className="text-muted-foreground mb-4">
              The BMAD Analyst Assistant encountered an error.
            </p>
      ...
      <Card className="flex flex-col h-[calc(100vh-12rem)]">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-2">
              ...
              <Switch
                ...
                disabled={useFallbackMode || !archonService}
              />
              <Badge 
                variant={useFallbackMode ? "outline" : archonService ? "default" : "destructive"} 
                className="ml-2"
              >
        ...
        {useFallbackMode && (
          <div className="px-4">
            <Alert variant="default" className="mb-2 border-orange-200 bg-orange-50">
              <AlertTriangle className="h-4 w-4 text-orange-600" />
              <AlertTitle className="text-orange-600">Fallback Mode Active</AlertTitle>
              <AlertDescription className="text-orange-700">
                Operating without Archon knowledge base access. Responses are based on general knowledge only.
              </AlertDescription>
            </Alert>
          </div>
        )}
